...
pub fn price_for_at(model: &str, prompt_tokens: u64, ts_millis: u64) -> Rates {
    let Some(m) = find_pricing(model) else {
        return FALLBACK_PRICE;
    };
    let tier_rates = select_tier(m, prompt_tokens).rates;
    match &m.time_of_day {
...
/// 选取输入 token 对应的价格档位（档位按 max_context 升序，最后一个通常无上限）。
fn select_tier(m: &ModelPricing, prompt_tokens: u64) -> Tier {
    for t in &m.tiers {
        if t.max_context.map(|c| prompt_tokens <= c).unwrap_or(true) {
            return t.clone();
        }
    }
    m.tiers.last().cloned().unwrap_or(Tier {
        max_context: None,
        rates: FALLBACK_PRICE,
    })
}

/// 把内置计费表序列化为 JSON，供前端展示模型能力/折扣/免费等信息。
...
mod tests {
    ...
    fn provider_prefix_and_case_insensitive() {
        ...
        let a = find_pricing("deepseek/deepseek-v4-flash").unwrap();
        let b = find_pricing("deepseek-v4-flash").unwrap();
        assert_eq!(a.id, b.id);
        let c = find_pricing("moonshotai/Kimi-K2.6").unwrap();
    ...
    fn off_peak_and_peak_pricing() {
        let m = find_pricing("deepseek-v4-pro").unwrap();
    ...
    fn free_models_cost_zero() {
        ...
        for id in ["laguna-s-2.1-free", "longcat-2.0:free"] {
            ...
            assert!(find_pricing(id).unwrap().deal.as_ref().unwrap().free);
        }
    }
    ...
    fn caps_exposed() {
        ...
        let k = find_pricing("kimi-k3").unwrap();
        assert!(k.caps.text && k.caps.vision && k.caps.reasoning);
        let m = find_pricing("minimax-m2.5").unwrap();
